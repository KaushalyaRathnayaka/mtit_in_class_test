...
public class UtilTRANSFORM extends UtilC {
	...
	public static ArrayList<Map<String, String>> XMLXPATHS() throws Exception {
		...
		for (int i = 1; i <= n; i++) {
			...
			m.put(CommonConstant.EMPLOYEE_ID, (String) x.compile("//Employees/Employee[" + i + "]/EmployeeID/text()")
					.evaluate(d, XPathConstants.STRING));
			m.put(CommonConstant.EMPLOYEE_NAME, (String) x.compile("//Employees/Employee[" + i + "]/EmployeeFullName/text()")
					.evaluate(d, XPathConstants.STRING));
			m.put(CommonConstant.EMPLOYEE_ADDRESS,
					(String) x.compile("//Employees/Employee[" + i + "]/EmployeeFullAddress/text()").evaluate(d,
							XPathConstants.STRING));
			m.put( CommonConstant.EMPLOYEE_FACULTY_NAME, (String) x.compile("//Employees/Employee[" + i + "]/FacultyName/text()")
					.evaluate(d, XPathConstants.STRING));
			m.put(CommonConstant.DEPARTEMENT, (String) x.compile("//Employees/Employee[" + i + "]/Department/text()")
					.evaluate(d, XPathConstants.STRING));
			m.put(CommonConstant.DESIGNATION, (String) x.compile("//Employees/Employee[" + i + "]/Designation/text()")
					.evaluate(d, XPathConstants.STRING));
			l.add(m);
		}
		return l;
